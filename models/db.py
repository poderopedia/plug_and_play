...
db.define_table(
    'auth_user',
    Field('username', type='string', label=T('Nombre de Usuario')),
    Field('first_name', type='string', label=T('Nombre')),
    Field('last_name', type='string', label=T('Apellido')),
...
db.define_table('caso',
                ...
                Field('country',db.country, label=T('País de Residencia'),
                    requires=IS_IN_DB(db, 'country.id', 'country.name', T('Seleccionar pais')), represent=lambda id, r: db.country[id].name if id else "") ,
